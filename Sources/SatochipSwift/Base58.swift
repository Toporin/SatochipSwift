// from https://github.com/keefertaylor/Base58Swift/blob/master/Base58Swift/Base58.swift
...
public enum Base58 {
  ...
  private static func calculateChecksum(_ input: [UInt8]) -> [UInt8] {
    let hashedBytes = Crypto.shared.sha256(input)
    let doubleHashedBytes = Crypto.shared.sha256(hashedBytes)
    return Array(doubleHashedBytes.prefix(checksumLength))
  }
    
//    private static func calculateChecksumOld(_ input: [UInt8]) -> [UInt8] {
